...
vmean_eve = 4; % E speed average
tmean_eve = pi/2; % E direction average
vsigma_eve = 0.5; % standard deviation of E speed
tsigma_eve = pi/4; % standard deviation of E direction

alpha_eve = 0.6; % memory level: 1=markov, 0=gauss
speed_eve = zeros([position,1]);
theta_eve = zeros([position,1]);
speed_eve(1) = rand+vmean_eve;
theta_eve(1) = rand*tmean_eve;
...
for t = 2:position
    ...
    % gauss markov
    speed_eve(t)=(alpha_eve*speed_eve(t-1))+((1-alpha_eve)*vmean_eve)+...
        (vsigma_eve*sqrt((1-(alpha_eve^2))*normrnd(vmean_eve,vsigma_eve)));
    theta_eve(t)=wrapTo2Pi(abs((alpha_eve*theta_eve(t-1))+((1-alpha_eve)*tmean_eve)+...
        (tsigma_eve*sqrt((1-(alpha_eve^2))*normrnd(tmean_eve,tsigma_eve)))));
